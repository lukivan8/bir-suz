# Bir Söz

Bir Söz — Chrome-расширение для ненавязчивого повторения слов во время обычного браузинга. Оно иногда показывает небольшую карточку с вопросом по словарю, вместо того чтобы заставлять открывать отдельное приложение для учёбы.

Цель проекта — помогать регулярно возвращаться к казахскому языку без отдельного учебного приложения. Сейчас фокус — словарь казахских слов с вариантами ответа для быстрого повторения.

## Установка

Расширение загружается из папки `dist/`. Готовая сборка в репозиторий не включена — её нужно собрать самостоятельно.

```bash
npm install
npm run build
```

Затем:

1. Откройте `chrome://extensions`.
2. Включите **Developer mode** / «Режим разработчика».
3. Нажмите **Load unpacked** / «Загрузить распакованное расширение».
4. Выберите папку `dist/`.
5. Откройте любую HTTP(S)-страницу и проверьте расширение через кнопку **Demo Trigger** во всплывающем окне расширения.

## Как это работает

- Карточка может появляться при открытии новых вкладок или переходах по ссылкам.
- Вопрос можно пропустить кнопкой **Skip** или клавишей **Escape**.
- Ответы и скорость ответа влияют на расписание повторения слов.
- Настройки и прогресс хранятся локально в `chrome.storage.local`.
- Если включена статистика использования, расширение отправляет технические
  события обучения на `https://api.lukivan8.com`: случайный идентификатор
  установки, выбранный словарь/слово, факт ответа или пропуска, корректность,
  время ответа и прогресс повторения. Расширение не отправляет адреса
  посещённых страниц, адреса ссылок, текст ссылок, CSS-селекторы, содержимое
  сайтов, cookies, пароли или историю браузера.
- Пользовательские словари в аналитике учитываются только как обобщённый
  прогресс с идентификатором `custom`: названия пользовательских словарей,
  пользовательские слова и их внутренние ID не отправляются.
- Статистика использования выключена по умолчанию. Её можно включить при первом
  запуске или в настройках расширения, а затем снова отключить в любой момент.
- Внутренняя статистика доступна на странице `dashboard.html`.

## Важные ограничения

- Это MVP, а не полноценный учебный продукт.
- Контент сейчас сфокусирован на казахском словаре.
- На некоторых страницах Chrome не разрешает внедрять расширения; в таком случае показ откладывается до следующей подходящей HTTP(S)-страницы.
- Учебный прогресс хранится локально и не синхронизируется между устройствами.
- Для публикации в Chrome Web Store нужно указать публичную политику
  конфиденциальности. Черновик находится в
  [`docs/privacy-policy.md`](docs/privacy-policy.md).

## Команды

```bash
npm run dev
npm run build
npm run lint
npm run check:write
```

## Демо-данные

После сборки откройте `dashboard.html` расширения, откройте DevTools Console и
выполните:

```js
await import(chrome.runtime.getURL('demo-seed.js')).then(({ seedDemo }) => seedDemo())
```

Скрипт заменяет текущий учебный прогресс: делает примерно половину слов в
каждом словаре освоенными, создаёт непрерывную активность за последние 30 дней
и отключает аналитику. Очереди ранее собранной аналитики также очищаются.

## Встроенные словари

Встроенные словари собираются из CSV-файлов в
[`src/shared/builtin-vocabularies`](src/shared/builtin-vocabularies). Один
`.csv` файл равен одному встроенному словарю. После добавления или замены CSV
нужно пересобрать расширение:

```bash
npm run build
```

Минимальный формат:

```csv
# name: Семья
# category: nouns
отбасы,семья
әке,отец
```
